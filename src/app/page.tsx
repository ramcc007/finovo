...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Hero */}
      <div className="bg-white border-b border-[#E5E4E0] py-12 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-[28px] font-bold text-[#1A1917] mb-2">
            Screen, Analyse &amp; Research Indian Stocks
          </h1>
          <p className="text-[#6B6966] text-base mb-8">
            Fundamental data for 5000+ NSE &amp; BSE listed companies — completely free.
          </p>

          <div className="relative max-w-[560px] mx-auto">
            <div className="flex items-center gap-3 bg-[#F8F7F4] border border-[#E5E4E0] rounded-xl px-5 py-3.5 shadow-sm">
              <Search size={18} className="text-[#9C9894] shrink-0" />
              <input
                type="text"
                ...
                className="bg-transparent text-[15px] text-[#1A1917] placeholder:text-[#9C9894] outline-none flex-1"
              />
            </div>

            {searchResults.length > 0 && (
              <div className="absolute top-full mt-2 left-0 right-0 bg-white border border-[#E5E4E0] rounded-[10px] shadow-md overflow-hidden z-50 text-left">
                {searchResults.map(s => (
                  <Link
                    key={s.symbol}
                    href={`/stocks/${s.symbol}`}
                    onClick={() => setQuery('')}
                    className="flex items-center justify-between px-5 py-3 hover:bg-[#EEF2FF] transition-colors"
                  >
                    <div>
                      <span className="text-sm font-semibold text-[#1A1917]">{s.symbol}</span>
                      <span className="text-xs text-[#6B6966] ml-2">{s.name}</span>
                    </div>
          ...
          <div className="flex items-center justify-center gap-2 mt-4 flex-wrap">
            {['TCS', 'RELIANCE', 'HDFCBANK', 'BAJFINANCE', 'INFY'].map(sym => (
              <Link key={sym} href={`/stocks/${sym}`} className="text-xs text-[#4F46E5] bg-[#EEF2FF] px-3 py-1 rounded-full hover:bg-[#E0E7FF] transition-colors font-medium">
                {sym}
              </Link>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 card p-0 overflow-hidden">
            <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-[#EEEDE9]">
              <h2 className="font-semibold text-[#1A1917] text-sm">Market Movers</h2>
                ...
                {(['gainers', 'losers'] as const).map(tab => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === tab ? 'bg-[#4F46E5] text-white' : 'text-[#6B6966] hover:bg-[#F1F0ED]'
                    )}
                  >
                ...
                {stocks.map(s => (
                  <tr key={s.symbol} onClick={() => router.push(`/stocks/${s.symbol}`)}>
                    <td>
                      ...
                      <div className="text-[11px] text-[#9C9894] font-sans mt-0.5">{s.name}</div>
                    </td>
                    <td>₹ {formatPrice(s.price)}</td>
                    <td><ChangeBadge value={s.changePct} /></td>
                    <td className="text-[#6B6966]">{s.volume}</td>
                    <td className="text-[#6B6966]">{s.marketCap}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-4">
            <div className="card p-5">
              <h2 className="font-semibold text-[#1A1917] text-sm mb-4">Market Breadth</h2>
              <div className="space-y-3">
                {[
                  { label: 'Advances', value: 1284, color: '#16A34A' },
                  { label: 'Declines', value: 932, color: '#DC2626' },
                  { label: 'Unchanged', value: 124, color: '#9C9894' },
                  { label: '52W Highs', value: 47, color: '#4F46E5' },
                  ...
                  <div key={item.label} className="flex items-center justify-between">
                    ...
                    <span className="num text-sm font-semibold text-[#1A1917]">{item.value.toLocaleString('en-IN')}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 h-1.5 rounded-full overflow-hidden flex">
                <div className="h-full bg-[#16A34A]" style={{ width: '57%' }} />
                <div className="h-full bg-[#9C9894]" style={{ width: '6%' }} />
                <div className="h-full bg-[#DC2626]" style={{ width: '37%' }} />
              </div>
              <div className="flex justify-between text-[10px] text-[#9C9894] mt-1">
                <span>Bullish</span><span>Bearish</span>
              </div>
            </div>

            <div className="card-plain p-4 space-y-2">
              <h3 className="text-xs font-semibold text-[#6B6966] uppercase tracking-wide mb-3">FII / DII Activity</h3>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#6B6966]">FII Net</span>
                <span className="num text-sm font-semibold text-[#16A34A]">+₹1,248 Cr</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-[#6B6966]">DII Net</span>
        ...
        {/* Sector Heatmap */}
        <div className="card p-5">
          <h2 className="font-semibold text-[#1A1917] text-sm mb-4">Sector Performance Today</h2>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[17px] font-semibold text-[#1A1917]">Popular Screens</h2>
            <Link href="/screens" className="text-sm text-[#4F46E5] hover:underline flex items-center gap-1">
            ...
            {PRE_BUILT_SCREENS.map(screen => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className="num text-xs font-semibold px-2 py-0.5 rounded-full bg-[#F1F0ED] text-[#6B6966]">{screen.count}</span>
                </div>
                <p className="text-xs text-[#6B6966] mb-3 leading-relaxed">{screen.description}</p>
                <p className="text-[11px] text-[#9C9894] font-mono">{screen.filters}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pb-8">
          {[
            ...
            <div key={stat.label} className="card-plain p-4 text-center">
              <div className="num text-2xl font-bold text-[#4F46E5] mb-1">{stat.value}</div>
              <div className="text-xs text-[#6B6966]">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
